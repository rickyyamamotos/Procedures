...
#              THE LOCATION OF wpconfig.php. IT COULD BE ANY SUBDIRECTORY /var/www/html/ (all VHOST root directories must be a subdirectory of /var/www/html/)
...
#FIND THE LOCATION OF wpconfig.php. IT COULD BE ANY SUBDIRECTORY /var/www/html/
for i in $(find /var/www/html/ -type f -name "wp-config.php"); do 
   vfilename1=$i
   if [ -f $vfilename1 ] ; then
      vwwwroot1=${vfilename1%/*}
      vwwwroot1=${vwwwroot1##*/}
#      echo "vfilename1 vwwwroot1: $vfilename1 $vwwwroot1"
   else
...
#   vwwwroot1="$(echo $vwwwroot1 | awk -F/ '{ print $3}')"
   echo "vwwwroot1 vdbtype1 vdbhost1 vdbname1 <Username> vdbpass1 vtabpre1: $vwwwroot1 $vdbtype1 $vdbhost1 $vdbname1 $<Username> $vdbpass1 $vtabpre1"
   ...
   if [ $vdmysql1 -eq $vdow1 -o $vdmysql1 -eq 8 ] ; then
      ...
      if [ $vexitcode1 -ne 0 ] ; then
         ...
         vtime1=$(TZ=US/Eastern date '+%H:%M:%S') 
         tar -cpzf /tmp/$vfilename1 /tmp/$vdbname1-backup.sql
         vexitcode1=$?
         rm -f /tmp/$vdbname1-backup.sql
         if [ $vexitcode1 -ne 0 ] ; then
   ...
   if [ $vdhtmlroot -eq $vdow1 -o $vdhtmlroot -eq 8 ] ; then
      ...
      tar -cpzf /tmp/$vfilename1 /var/www/html --exclude='/var/www/html/wp-content/updraft'
      vexitcode1=$?
      if [ $vexitcode1 -ne 0 ] ; then
         echo "tar failed with exit code $vexitcode1"
         rm -f /tmp/$vfilename1
         sendEmail -vv -o tls=no -m "Error on $vdate1 when compressing /var/www/html on $vwwwroot1($vip1):/tmp/$vfilename1-> tar failed with exit code $vexitcode1" -f <Email> -t <Email> -cc <Email> -s smtp.gmail.com:587 -xu <Email> -xp <Password> -u "$vwwwroot1($vip1):/wordpress-backup.sh tar Error: $vexitcode1" >> /var/log/sendEmail.log
