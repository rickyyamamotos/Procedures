...
#PURPOSE: backup passenger-rails app data to the drobo in the <Location> Office. POSTGRESQL ONLY
...
#              #Asuming the database is hosted locally
#              sudo crontab -e
...
railsenv1=""
#OLD: railsenv1=$(passenger-status | grep "):")
#OLD: railsenv1=${railsenv1:`expr index "$railsenv1" "\("`}
#OLD: railsenv1=${railsenv1:0:`expr index "$railsenv1" "\)" - 1`}
railsenv1=$(cat /etc/*-release | grep "centos")
if [ -z "$railsenv1" ] ; then
   railsenv1=$(cat /etc/*-release | grep "ubuntu")
   if [ -z "$railsenv1" ] ; then
      sendEmail -vv -o tls=yes -m "Error on $vdate1 when determining the rails environment, OS not ubuntu neither centos. Check file: $vfilename1" -f <Email> -t <Email> -cc <Email> -s smtp.gmail.com:587 -xu <Email> -xp <Password> -u "$vwwwroot1($vip1):/passenger-backup.sh cant determine rails environment" >> /var/log/sendEmail.log
      sshpass -p '<Password>' ssh -p <PortNumber> <Username>@XXX.XXX.XXX.243 "/bin/echo $vdate1 $vtime1 $(date '+%Y-%m-%d') $(TZ=US/Eastern date '+%H:%M:%S') EDT Error determining the rails environment, OS is neither centos or ubuntu. Check file: $vfilename1 >> /mnt/shares/drobo/backupserver.log"
      exit
   else
      railsenv1=$(cat /etc/apache2/sites-enabled/canvas.conf | grep "SetEnv RAILS_ENV" -m 1)
      railsenv1=${railsenv1:`expr index "$railsenv1" "V"`}
   fi
else
   railsenv1=$(cat /etc/httpd/conf.d/canvas.conf | grep "SetEnv RAILS_ENV" -m 1)
   railsenv1=${railsenv1:`expr index "$railsenv1" "V"`}
fi
...
#CREATING, COMPACTING AND TRANFERING  A DATABASE DUMP FOR POSTGRESQL ONLY
if [ $vdmysql1 -eq $vdow1 -o $vdmysql1 -eq 8 ] && [ `expr match "$vdbtype1" "postgresql"` \> 0 ] ; then
   echo "Started the creation the database dump..."
   if [ `expr match "$vdbtype1" "postgresql"` \> 0 ] ; then
      vtime1=$(TZ=US/Eastern date '+%H:%M:%S')
      sshpass -p $vdbpass1 /usr/bin/pg_dump --file=/tmp/$vdbname1-backup.pg_dump -Fc -v -b -o -O -x --host=$vdbhost1 --username=$<Username> --dbname=$vdbname1
      vexitcode1=$?
