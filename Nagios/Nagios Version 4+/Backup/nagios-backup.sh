#!/bin/bash
#SCRIPT NAME: /nagios-backup.sh
#PURPOSE: backup nagios configuration data to the drobo in the <Location> Office
#CREATED ON: 07/11/2019
#CREATED BY: Rick Yamamoto
#VERSION: 1 of 07/11/2019
#INTRUCTIONS: copy this script onto the root of the nagios server. Then create a crontab job to autoexecute it everyday
...
#              sudo crontab -e
#                 01 03 *   *   *    /bin/bash /nagios-backup.sh
#              #  Mn Hr DoM Mon DoW
...
#   vddataroot  defines which days to make a backup of the nagios's configuration directories
...
#COMPATING AND TRANSMITTING THE NAGIOS CONFIGURATION DIRECTORIES
#SET VALUE OF vddataroot ON THE TOP OF THIS SCRIPT
if [ $vddataroot -eq $vdow1 -o $vddataroot -eq 8 ] ; then
   echo "Started compressing the nagios configuration directories"
   vtime1=$(TZ=US/Eastern date '+%H:%M:%S')
   vfilename1=$vdate1-dataroot-backup.tar.gz
   tar --exclude="/usr/local/nagios/var" cpPzf /tmp/$vfilename1 /usr/local/nagios/ /usr/lib64/nagios/plugins/ 
   vexitcode1=$?
   if [ $vexitcode1 -ne 0 ] ; then
      echo "tar failed with exit code $vexitcode1"
      rm -f /tmp/$vfilename1
      sendEmail -vv -o tls=yes -m "Error on $vdate1 when compressing the nagios configuration directories on $vwwwroot1($vip1):/tmp/$vfilename1-> tar failed with exit code $vexitcode1" -f <Email> -t <Email> -cc <Email> -s smtp.gmail.com:587 -xu <Email> -xp <Password> -u "$vwwwroot1($vip1):/nagios-backup.sh tar Error: $vexitcode1" >> /var/log/sendEmail.log
      ...
      if [ $vexitcode1 -ne 0 ] ; then
         ...
         sendEmail -vv -o tls=yes -m "Error on $vdate1 when transmitting the nagios config directories from $vwwwroot1($vip1) to <Location>\\\\XXX.XXX.XXX.87\\backup\\$vwwwroot1\\$vfilename1.\n -> sshpass failed with exit code $vexitcode1" -f <Email> -t <Email> -cc <Email> -s smtp.gmail.com:587 -xu <Email> -xp <Password> -u "$vwwwroot1($vip1):/nagios-backup.sh sshpass Error: $verrorcdoe1" >> /var/log/sendEmail.log
         ...
         sendEmail -vv -o tls=yes -m "Successfully backed up $vwwwroot1($vip1) dataroot directory  to:\n   <Location>\\\\XXX.XXX.XXX.87\\backup\\$vwwwroot1\\$vfilename1.\n Date: $vdate1\n See <Location>Office\\\\XXX.XXX.XXX.87\\backup\\restore-procedure.txt for instructions on restore the backup" -f <Email> -t <Email> -s smtp.gmail.com:587 -xu <Email> -xp <Password> -u "$vwwwroot1($vip1):Successful backup of nagios config directories made on $vdate1" >> /var/log/sendEmail.log
